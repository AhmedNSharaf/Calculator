...
public class Cube implements ActionListener {
    JFrame frame;
    ...
    double result1=0,result2=0,length=0;
    Cube()
    {
        frame=new JFrame("Cube Calculator");
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        frame.setSize(420,550);
        frame.setLayout(null);


        text1=new JTextField("Enter the length of the side");
        text1.setBounds(40, 25, 330, 50);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if(e.getSource()==clrButton) {
			text2.setText("");
            text1.setText("Enter the length of the side");
		}
        ...
        if(e.getSource()==area) {
			length = Double.parseDouble(text2.getText());
			text2.setText("");
            result1=6*length*length;
            text2.setText(String.valueOf(result1));
            text1.setText("Area Of Cube Is ");
		}
		if(e.getSource()==Volume) {
            length = Double.parseDouble(text2.getText());
			text2.setText("");
            result1=length*length*length;
            text2.setText(String.valueOf(result1));
            text1.setText("Volume Of Cube is ");
		}
		if(e.getSource()==areaAndVolume) {
            length = Double.parseDouble(text2.getText());
			text2.setText("");
            result1=6*length*length;
            result2=length*length*length;
